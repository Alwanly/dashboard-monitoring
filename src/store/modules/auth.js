   ...
   const actions= {
       async login({commit},user){
        ...
        const requestLogin = {
           ...
           try{            
               const response  = await ApiServices.customRequest(requestLogin).catch((err) => commit('auth_error',err))
               Services.saveToken(response.data.token)
               ApiServices.setHeader()
               const getProfile = await ApiServices.get('user/me')
               .catch((err) =>{ 
               ...
               commit('auth_success',{token:Services.getToken(),user:Services.getUser()})
